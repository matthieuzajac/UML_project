...
(Clone de Royal Road)}
\author{Prénom NOM \quad -- \quad Prénom NOM \quad -- \quad Prénom NOM}
\date{\today}
...
\subsection{Correspondance UML -- C++}

L'implémentation C++ est fournie dans le dossier \texttt{cpp\_implementation} avec les fichiers d'en-tête pour chaque classe du diagramme et des fichiers \texttt{.cpp} contenant les signatures de méthodes. Un \texttt{main.cpp} instancie l'ensemble des objets modélisés afin de valider la cohérence UML--C++ et garantir une base compilable conforme aux consignes.
